...
def decision_tree_node(df, features):
    ...
    Returns:
    - best_split_param: optimal age value for splitting
    ...
    # Don't filter out missing age values (-1)
    # df_valid = df[['Survived','Age']].copy()
    # df_survived = df[['Survived']].copy()
...
# Perform decision tree analysis on DataFrame with respect to features
# return best_feature, best_split_param_global, best_gini_global, left_group_best, right_group_best
depth=2
df_left,df_right = decision_tree_node(processed_df, features)[3:5]
left_branch=[]
right_branch=[]
left_branch.append(df_left)
right_branch.append(df_right)
print(len(left_branch))
for i in range(depth):
    for l in range(len(left_branch)):
        l1,r1 = decision_tree_node(df_left, features)[3:5]
        left_branch.append(l1)
        left_branch.append(r1)
    for r in range(len(right_branch)):
        l2,r2 = decision_tree_node(df_right, features)[3:5]
        right_branch.append(l2)
        right_branch.append(r2)
    


# depth=5
# for d in range(depth):
#     decision_tree_node(left_group_best, features)

# # Test prediction with PassengerId
# predict_survival(PassengerId=101, age_threshold=age_condition, df=processed_df)
