...
def build_decision_tree(df, features, max_depth, gini_threshold, min_group, current_depth=0):
    ...
    if len(df) <= min_group:
        total_score = survival_counter.calc(df['Survived'].sum(), total_score, len(df), survival_r)
        print(f"Depth {current_depth}: Group of {min_group}, it is too small")
        print("=" * 60)
        return {
            'leaf': True,
            'depth': current_depth,
...
########## SINGLE SLICE
# Get a slice, usage
df, train_indices = df_random_slice(processed_df, percent=20, random_seed=21)
...
def train_trees(df, percent=20, sessions=20):
    accuracy_ratio_best = 0
    tree_best = None
    for session in range(sessions):
        df, train_indices = df_random_slice(processed_df, percent=percent, random_seed=None)
        tree = build_decision_tree(df, features, max_depth, gini_threshold=gini_threshold, min_group=min_group)
        tree_predictions = predict_batch(tree, df, features_all)
        accuracy_ratio = tree_predictions['accuracy'].sum()/len(tree_predictions)
        print(f'Accuracy of the tree No: {session+1} is {accuracy_ratio:.2%}')
        print("=" * 90)
        if accuracy_ratio > accuracy_ratio_best:
            print('better tree lol')
            accuracy_ratio_best = accuracy_ratio
            tree_best = tree
            df_trained = df
    return tree_best, df_trained


tree_best, df_trained = train_trees(processed_df, percent=40, sessions=20)
tree_predictions = predict_batch(tree_best, df_trained, features_all)
accuracy_ratio = tree_predictions['accuracy'].sum()/len(tree_predictions)
score = tree_predictions['accuracy'].sum()
print(f"Final score of the obtained tree on training data: {score} out of {tree_best['size']}, giving {accuracy_ratio:.2%} accuracy")
